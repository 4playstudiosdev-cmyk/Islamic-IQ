...
const CSS = `
  ...
  .ar::before { content:'';position:fixed;inset:0;background:radial-gradient(ellipse at 35% 45%,rgba(201,168,76,0.07) 0%,transparent 60%);pointer-events:none; }
  .ac { background:linear-gradient(135deg,rgba(18,18,16,0.97),rgba(10,10,8,0.98));border:1px solid rgba(201,168,76,0.14);border-radius:22px;padding:42px 38px;width:100%;max-width:400px;position:relative;z-index:1;box-shadow:0 40px 80px rgba(0,0,0,0.7);animation:fadeUp 0.5s ease; }
  ...
  .ai:focus { border-color:rgba(201,168,76,0.45);background:rgba(201,168,76,0.03); }
  ...
  .gg:hover { border-color:rgba(201,168,76,0.28);background:rgba(201,168,76,0.04); }
  .div { display:flex;align-items:center;gap:10px;margin:16px 0; }
  .div::before,.div::after { content:'';flex:1;height:1px;background:rgba(242,237,228,0.07); }
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const { data: profile } = await supabase
        ...
        .eq('id', data.user.id)
        .single();
      if (!profile?.full_name) navigate('/setup-profile');
      else navigate('/home');
    } catch (e) {
    ...
    <div className="ar">
      ...
      <div className="ac">
        ...
        <div style={{textAlign:'center',marginBottom:30}}>
          ...
          <div style={{fontSize:11,color:'rgba(242,237,228,0.35)',letterSpacing:1.5}}>WELCOME BACK</div>
        </div>
        ...
        <div className="div"><span style={{fontSize:11,color:'rgba(242,237,228,0.22)'}}>or sign in with email</span></div>

        <form onSubmit={handleLogin} style={{display:'flex',flexDirection:'column',gap:11}}>
          <div>
            <label style={{fontSize:10,color:'rgba(242,237,228,0.38)',letterSpacing:1.2,display:'block',marginBottom:6}}>EMAIL</label>
            <input className="ai" type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="your@email.com"/>
          </div>
          <div>
            <div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}>
              <label style={{fontSize:10,color:'rgba(242,237,228,0.38)',letterSpacing:1.2}}>PASSWORD</label>
              <span style={{fontSize:11,color:'rgba(201,168,76,0.55)',cursor:'pointer'}}>Forgot?</span>
            </div>
            <input className="ai" type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="••••••••"/>
          </div>
          ...
          <button className="bg" type="submit" disabled={loading} style={{marginTop:4}}>{loading?'Signing in...':'SIGN IN'}</button>
        </form>

        <button onClick={() => navigate('/home')} style={{width:'100%',padding:'11px',background:'transparent',border:'1px solid rgba(242,237,228,0.05)',borderRadius:10,color:'rgba(242,237,228,0.3)',fontSize:12,cursor:'pointer',marginTop:10,transition:'all 0.3s'}}
          onMouseEnter={e=>e.target.style.borderColor='rgba(201,168,76,0.15)'}
          onMouseLeave={e=>e.target.style.borderColor='rgba(242,237,228,0.05)'}
        >Continue as Guest →</button>

        <div style={{textAlign:'center',marginTop:18,fontSize:12,color:'rgba(242,237,228,0.28)'}}>
          No account?{' '}<Link to="/signup" style={{color:'#C9A84C',textDecoration:'none',fontWeight:600}}>Sign Up</Link>
        </div>
      </div>
    </div>
  );
