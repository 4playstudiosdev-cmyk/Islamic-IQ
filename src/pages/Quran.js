...
import { getSurahArabic, getSurahEnglish, getSurahUrdu, getAyatAudioUrl, speakUrduTTS } from '../services/quranApi';
...
const QCSS = `
  ...
  :root { --qgold:#C9A84C; --qgold2:#E8C97A; --qbg:#030303; --qcard:#0F0F0D; --qcard2:#0A0A08; --qborder:rgba(201,168,76,0.12); --qtext:rgba(242,237,228,0.85); --qmuted:rgba(242,237,228,0.4); }
  .q-root { background:var(--qbg); color:var(--qtext); font-family:'Plus Jakarta Sans',sans-serif; }
  .q-sidebar { background:var(--qcard2) !important; border-right:1px solid var(--qborder) !important; }
  .q-tab-active { background:rgba(201,168,76,0.15) !important; color:var(--qgold) !important; }
  .q-surah-active { background:rgba(201,168,76,0.08) !important; border-left:3px solid var(--qgold) !important; }
  .q-ayat-card { background:var(--qcard) !important; border:1px solid var(--qborder) !important; border-radius:16px !important; margin-bottom:12px !important; }
  .q-ayat-card.playing { background:rgba(201,168,76,0.06) !important; border-color:rgba(201,168,76,0.3) !important; box-shadow:0 4px 20px rgba(201,168,76,0.1) !important; }
  .q-arabic { color:var(--qgold2) !important; font-size:clamp(20px,2.5vw,26px) !important; }
  .q-english { color:var(--qmuted) !important; font-style:italic; line-height:1.8 !important; }
  .q-header { background:linear-gradient(135deg,#050505,#0F0F0D,#050505) !important; border-bottom:1px solid var(--qborder) !important; }
  .q-bismillah { color:var(--qgold) !important; font-size:24px !important; }
  .q-num { background:rgba(201,168,76,0.1) !important; border:1px solid rgba(201,168,76,0.2) !important; color:var(--qgold) !important; border-radius:8px !important; }
  .q-play-btn { background:rgba(201,168,76,0.1) !important; border:1px solid rgba(201,168,76,0.2) !important; color:var(--qgold) !important; border-radius:8px !important; }
  .q-play-btn:hover { background:rgba(201,168,76,0.2) !important; }
  .q-audio-bar { background:linear-gradient(135deg,#050505,#0F0F0D) !important; border-top:1px solid var(--qborder) !important; }
  @keyframes qfadeIn { from{opacity:0;transform:translateY(10px)} to{opacity:1;transform:translateY(0)} }
  .q-ayat-card { animation:qfadeIn 0.3s ease both; }
...
export default function Quran() {
  ...
  const [zTest, setZTest]                 = useState({ loading:false, result:null, error:null });
  const [urduTTSLoading, setUrduTTSLoading] = useState(null); // ayat index being loaded
  const urduAudioRef = useRef(new Audio());
  ...
  return (
    <div className="q-root" style={{ display: 'flex', height: isMobile ? 'calc(100vh - 120px)' : '100vh', overflow: 'hidden', background:'#030303' }}>
      <style>{QCSS}</style>
      ...
      {isMobile && drawerOpen && (
        <div style={{ position:'fixed', inset:0, zIndex:500 }} onClick={() => setDrawerOpen(false)}>
          ...
          <div onClick={e => e.stopPropagation()} style={{ position:'absolute', top:0, left:0, bottom:0, width:'82%', maxWidth:310, background:'#0A0A08', borderRight:'1px solid rgba(201,168,76,0.12)', display:'flex', flexDirection:'column', overflow:'hidden' }}>
            {/* Close button */}
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'14px 14px 0' }}>
              <h1 style={{ fontSize:16, fontWeight:800, display:'flex', alignItems:'center', gap:6 }}>
                <BookOpen size={16} color="#C9A84C"/> <span className="gold-shimmer">القرآن الكريم</span>
              </h1>
              <button onClick={() => setDrawerOpen(false)} style={{ background:'rgba(255,255,255,0.08)', border:'none', borderRadius:8, padding:'5px 9px', cursor:'pointer', color:'rgba(242,237,228,0.4)', fontSize:14 }}>✕</button>
            </div>
            {/* Surah/Parah tabs */}
            <div style={{ padding:'12px 14px', borderBottom:'1px solid rgba(201,168,76,0.1)' }}>
              <div style={{ display:'flex', gap:6, marginBottom:10, background:'rgba(201,168,76,0.06)', borderRadius:10, padding:4 }}>
                {['surah','parah'].map(t => (
                  <button key={t} onClick={() => { setActiveTab(t); setSelectedParah(null); }} style={{ flex:1, padding:'7px', borderRadius:7, border:'none', background: activeTab===t ? 'rgba(201,168,76,0.15)' : 'transparent', color: activeTab===t ? '#C9A84C' : 'rgba(242,237,228,0.35)', fontSize:12, fontWeight: activeTab===t ? 600 : 400, cursor:'pointer' }}>
                    {t === 'surah' ? '📖 Surah' : '📚 Parah'}
                  </button>
                ))}
              </div>
              {activeTab === 'surah' && (
                <div style={{ position:'relative' }}>
                  ...
                  <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search surah..."
                    style={{ width:'100%', padding:'8px 10px 8px 30px', background:'rgba(201,168,76,0.06)', border:'1px solid rgba(201,168,76,0.15)', borderRadius:8, color:'white', fontSize:16, outline:'none', fontFamily:'inherit' }}/>
                </div>
              )}
            </div>
            {/* Surah/Parah list */}
            <div style={{ flex:1, overflowY:'auto' }}>
              {loadingSurahs ? (
                <div style={{ padding:40, textAlign:'center' }}>
                  <Loader size={22} color="#C9A84C" style={{ animation:'spin 1s linear infinite' }}/>
                  <p style={{ color:'rgba(242,237,228,0.3)', fontSize:11, marginTop:10 }}>Loading...</p>
                </div>
                ...
                return (
                  <button key={s.number} onClick={() => { stopAll(); S.current.isParah=false; setSelected(s); setDrawerOpen(false); }} style={{ width:'100%', padding:'11px 14px', background: isA ? 'rgba(201,168,76,0.08)' : 'transparent', border:'none', borderLeft: isA ? '3px solid #2E8B57' : '3px solid transparent', cursor:'pointer', textAlign:'left', display:'flex', alignItems:'center', gap:10 }}>
                    <div style={{ width:30, height:30, flexShrink:0, background: isA ? 'rgba(201,168,76,0.2)' : 'rgba(201,168,76,0.06)', borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center', fontSize:10, fontWeight:700, color: isA ? '#3aad6e' : '#4a6355' }}>{s.number}</div>
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ display:'flex', justifyContent:'space-between' }}>
                        <span style={{ fontSize:12, fontWeight:600, color: isA ? '#E8C97A' : 'rgba(242,237,228,0.7)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:100 }}>{s.englishName}</span>
                        <span className="arabic" style={{ fontSize:13, color:'#C9A84C' }}>{s.name}</span>
                      </div>
                      <div style={{ fontSize:10, color:'rgba(242,237,228,0.3)' }}>{s.numberOfAyahs} ayats</div>
                    </div>
                  </button>
                );
                ...
                <button key={p.num} onClick={() => setSelectedParah(p)} style={{ width:'100%', padding:'12px 14px', background:'transparent', border:'none', borderLeft:'3px solid transparent', cursor:'pointer', textAlign:'left', display:'flex', alignItems:'center', gap:10 }}>
                  <div style={{ width:34, height:34, flexShrink:0, background:'rgba(212,175,55,0.1)', borderRadius:9, display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:800, color:'#C9A84C' }}>{p.num}</div>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:'flex', justifyContent:'space-between' }}>
                      <span style={{ fontSize:12, fontWeight:600, color:'rgba(242,237,228,0.7)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:110 }}>{p.name}</span>
                      <span className="arabic" style={{ fontSize:13, color:'#C9A84C' }}>{p.arabic}</span>
                    </div>
                    <div style={{ fontSize:10, color:'rgba(242,237,228,0.3)' }}>Surahs {p.startSurah}–{p.endSurah}</div>
                  </div>
                </button>
              )) : (
                <>
                  <button onClick={() => setSelectedParah(null)} style={{ width:'100%', padding:'10px 14px', background:'rgba(212,175,55,0.08)', border:'none', borderBottom:'1px solid rgba(212,175,55,0.15)', cursor:'pointer', textAlign:'left', color:'#C9A84C', fontSize:12, fontWeight:600 }}>← All Parahs</button>
                  <div style={{ padding:'14px 16px', borderBottom:'1px solid rgba(201,168,76,0.1)' }}>
                    <div className="arabic" style={{ fontSize:16, color:'#C9A84C', marginBottom:4 }}>{selectedParah.arabic}</div>
                  ...
                  {getParahSurahs(selectedParah.num).map(s => {
                    ...
                    return (
                      <button key={s.number} onClick={() => { stopAll(); S.current.isParah=false; setSelected(s); setDrawerOpen(false); }} style={{ width:'100%', padding:'10px 14px', background: isCur?'rgba(212,175,55,0.1)':'transparent', border:'none', borderLeft: isCur?'3px solid #D4AF37':'3px solid transparent', cursor:'pointer', textAlign:'left', display:'flex', alignItems:'center', gap:10 }}>
                        <div style={{ width:28, height:28, flexShrink:0, background: isCur?'rgba(212,175,55,0.25)':'rgba(201,168,76,0.06)', borderRadius:7, display:'flex', alignItems:'center', justifyContent:'center', fontSize:10, fontWeight:700, color: isCur?'#C9A84C':'#4a6355' }}>{s.number}</div>
                        <div style={{ flex:1, minWidth:0 }}>
                          <div style={{ display:'flex', justifyContent:'space-between' }}>
                            <span style={{ fontSize:12, fontWeight:600, color: isCur?'#f5d060':'#c0d4c8' }}>{s.englishName}</span>
                            <span className="arabic" style={{ fontSize:13, color:'#C9A84C' }}>{s.name}</span>
                          </div>
                          <div style={{ fontSize:10, color: lbl?'#C9A84C':'#4a6355' }}>{lbl?`📍 ${lbl}`:`${s.numberOfAyahs} ayats`}</div>
                        </div>
                      </button>
                    );
                  })}
      ...
      {/* ── DESKTOP: Sidebar ── */}
      {!isMobile && (
        <div className="q-sidebar" style={{ width:300, borderRight:'1px solid rgba(201,168,76,0.12)', display:'flex', flexDirection:'column', background:'#0A0A08', flexShrink:0, fontFamily:"'Plus Jakarta Sans',sans-serif" }}>
          <div style={{ padding:'14px', borderBottom:'1px solid rgba(201,168,76,0.1)', flexShrink:0 }}>
            <h1 style={{ fontSize:16, fontWeight:800, marginBottom:10, display:'flex', alignItems:'center', gap:6 }}>
              <BookOpen size={16} color="#C9A84C"/> القرآن الكريم
            </h1>
            <div style={{ display:'flex', gap:6, marginBottom:10, background:'rgba(201,168,76,0.06)', borderRadius:10, padding:4 }}>
              {['surah','parah'].map(t => (
                <button key={t} onClick={() => { setActiveTab(t); setSelectedParah(null); }} style={{ flex:1, padding:'7px', borderRadius:7, border:'none', background: activeTab===t ? 'rgba(201,168,76,0.15)' : 'transparent', color: activeTab===t ? '#C9A84C' : 'rgba(242,237,228,0.35)', fontSize:12, fontWeight: activeTab===t ? 600 : 400, cursor:'pointer' }}>
            ...
            {activeTab === 'surah' && (
              <div style={{ position:'relative' }}>
                <Search size={12} color="#4a6355" style={{ position:'absolute', left:10, top:'50%', transform:'translateY(-50%)' }}/>
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search surah..."
                  style={{ width:'100%', padding:'8px 10px 8px 30px', background:'rgba(201,168,76,0.06)', border:'1px solid rgba(201,168,76,0.15)', borderRadius:8, color:'white', fontSize:12, outline:'none', fontFamily:'inherit' }}/>
              </div>
            )}
          </div>
          <div style={{ flex:1, overflowY:'auto' }}>
            {loadingSurahs ? (
              <div style={{ padding:40, textAlign:'center' }}>
                <Loader size={22} color="#C9A84C" style={{ animation:'spin 1s linear infinite' }}/>
                <p style={{ color:'rgba(242,237,228,0.3)', fontSize:11, marginTop:10 }}>Loading Quran...</p>
              </div>
            ) : activeTab === 'surah' ? filtered.map(s => {
              const isA = selected?.number === s.number && !S.current.isParah;
              return (
                <button key={s.number} onClick={() => { stopAll(); S.current.isParah=false; setSelected(s); }} style={{ width:'100%', padding:'10px 14px', background: isA?'rgba(201,168,76,0.08)':'transparent', border:'none', borderLeft: isA?'3px solid #C9A84C':'3px solid transparent', cursor:'pointer', textAlign:'left', display:'flex', alignItems:'center', gap:10 }}
                ...
                >
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ fontSize:10, color:'rgba(242,237,228,0.3)', marginTop:1 }}>{s.numberOfAyahs} ayats · {s.revelationType}</div>
                  </div>
                </button>
              );
            }) : selectedParah === null ? PARAHS.map(p => {
              ...
              return (
                <button key={p.num} onClick={() => setSelectedParah(p)} style={{ width:'100%', padding:'12px 14px', background: isP?'rgba(212,175,55,0.1)':'transparent', border:'none', borderLeft: isP?'3px solid #D4AF37':'3px solid transparent', cursor:'pointer', textAlign:'left', display:'flex', alignItems:'center', gap:10 }}
                  onMouseEnter={e => { if(!isP){ e.currentTarget.style.background='rgba(212,175,55,0.06)'; e.currentTarget.style.borderLeftColor='#C9A84C'; }}}
                  onMouseLeave={e => { if(!isP){ e.currentTarget.style.background='transparent'; e.currentTarget.style.borderLeftColor='transparent'; }}}
                >
                  <div style={{ width:34, height:34, flexShrink:0, background: isP?'rgba(212,175,55,0.25)':'rgba(212,175,55,0.1)', borderRadius:9, display:'flex', alignItems:'center', justifyContent:'center', fontSize: isP?16:11, fontWeight:800, color:'#C9A84C' }}>{isP&&playing?'▶':p.num}</div>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:'flex', justifyContent:'space-between' }}>
                    ...
                    <div style={{ fontSize:10, color:'rgba(242,237,228,0.3)', marginTop:1 }}>Parah {p.num} · {p.startSurah}–{p.endSurah}</div>
                  </div>
                </button>
              );
            }) : (
              <>
                <button onClick={() => setSelectedParah(null)} style={{ width:'100%', padding:'10px 14px', background:'rgba(212,175,55,0.08)', border:'none', borderBottom:'1px solid rgba(212,175,55,0.15)', cursor:'pointer', textAlign:'left', color:'#C9A84C', fontSize:12, fontWeight:600, display:'flex', alignItems:'center', gap:6 }}>← All Parahs</button>
                <div style={{ padding:'14px 16px', borderBottom:'1px solid rgba(201,168,76,0.1)' }}>
                  <div className="arabic" style={{ fontSize:16, color:'#C9A84C', marginBottom:4 }}>{selectedParah.arabic}</div>
                  <div style={{ fontSize:13, fontWeight:700, color:'white', marginBottom:10 }}>Parah {selectedParah.num} — {selectedParah.name}</div>
                ...
                {getParahSurahs(selectedParah.num).map(s => {
                  ...
                  return (
                    <button key={s.number} onClick={() => { stopAll(); S.current.isParah=false; setSelected(s); }} style={{ width:'100%', padding:'10px 14px', background: isCur?'rgba(212,175,55,0.1)':'transparent', border:'none', borderLeft: isCur?'3px solid #D4AF37':'3px solid transparent', cursor:'pointer', textAlign:'left', display:'flex', alignItems:'center', gap:10 }}>
                      ...
                      {isCur&&playing&&<div style={{ display:'flex', gap:2 }}>{[0,1,2].map(i=><div key={i} style={{ width:3, height:12, background:'#C9A84C', borderRadius:2, animation:`pulse 0.8s ease ${i*0.2}s infinite` }}/>)}</div>}
                    </button>
                  );
                })}
              </>
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: playing ? 80 : 0 }}>
        {!selected ? (
          <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            <div className="arabic" style={{ fontSize: 52, color: '#C9A84C', marginBottom: 16 }}>﷽</div>
            <p style={{ fontSize: 14, color: 'rgba(242,237,228,0.3)', marginBottom: 8 }}>Select a Surah or Parah to begin</p>
            <p className="arabic" style={{ fontSize: 17, color: '#C9A84C', marginBottom: 20 }}>اقْرَأْ بِاسْمِ رَبِّكَ الَّذِي خَلَقَ</p>
            {isMobile && (
          ...
          <>
            <div style={{ background: 'linear-gradient(135deg, #050505, #0F0F0D, #050505)', padding: isMobile ? '12px 16px' : '18px 24px', borderBottom: '1px solid rgba(212,175,55,0.15)', position: 'sticky', top: 0, zIndex: 10 }}>
              {/* Mobile hamburger */}
              {isMobile && (
                <button onClick={() => setDrawerOpen(true)} style={{ background: 'rgba(212,175,55,0.15)', border: '1px solid rgba(212,175,55,0.3)', borderRadius: 9, padding: '7px 12px', cursor: 'pointer', color: '#C9A84C', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 7, fontSize: 12, fontWeight: 600 }}>
              ...
              {playingParah && (
                <div style={{ background: 'rgba(212,175,55,0.12)', border: '1px solid rgba(212,175,55,0.25)', borderRadius: 8, padding: '5px 12px', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ display: 'flex', gap: 2 }}>{[0,1,2].map(i => <div key={i} style={{ width: 3, height: 10, background: '#E8C97A', borderRadius: 2, animation: `pulse 1s ease ${i*0.2}s infinite` }} />)}</div>
                  <span style={{ fontSize: 11, color: '#C9A84C', fontWeight: 600 }}>Playing Parah {playingParah.num} — {playingParah.name}</span>
                </div>
              )}
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', background: 'rgba(0,0,0,0.25)', borderRadius: 8, padding: 3 }}>
                    {[{v:'english',l:'EN'},{v:'urdu',l:'اردو'},{v:'both',l:'Both'}].map(({v,l}) => (
                      <button key={v} onClick={() => setDisplayLang(v)} style={{ background: displayLang === v ? 'rgba(201,168,76,0.3)' : 'transparent', border: 'none', borderRadius: 6, padding: '5px 9px', color: displayLang === v ? 'white' : 'rgba(255,255,255,0.4)', cursor: 'pointer', fontSize: 11, fontWeight: displayLang === v ? 600 : 400 }}>{l}</button>
                    ))}
                  </div>
                  ...
                  {!playingParah ? (
                    <div style={{ display: 'flex', gap: 5 }}>
                      <button onClick={() => { if (playing && playMode === 'ayat') stopAll(); else startSurah('ayat'); }} style={{ background: playing && playMode === 'ayat' ? 'rgba(212,175,55,0.2)' : 'rgba(201,168,76,0.2)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 50, padding: '7px 14px', color: 'white', cursor: 'pointer', fontSize: 11, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5 }}>
            ...
            {/* ── Zyphra Test Button ── */}
            <div style={{ padding:'8px 26px', borderBottom:'1px solid rgba(201,168,76,0.08)' }}>
              <button
                onClick={async () => {
                  if (!urduTrans.length) return;
                  setZTest({ loading:true, result:null, error:null });
                  const testUrdu = urduTrans[0]?.text || 'اللہ کے نام سے';
                  console.log('🎵 Testing Zyphra TTS with Urdu:', testUrdu);
                  const blobUrl = await speakUrduTTS(testUrdu);
                  if (blobUrl) {
                    setZTest({ loading:false, result:'✅ Audio ready! Playing...', error:null });
                    urduAudioRef.current.src = blobUrl;
                    urduAudioRef.current.play().catch(console.error);
                  } else {
                    setZTest({ loading:false, result:null, error:'TTS failed — check console & .env key' });
                  }
                }}
                style={{ padding:'6px 14px', background:'rgba(201,168,76,0.1)', border:'1px solid rgba(201,168,76,0.3)', borderRadius:8, color:'#C9A84C', fontSize:11, cursor:'pointer', fontFamily:'inherit', display:'flex', alignItems:'center', gap:6 }}
              >
                {zTest.loading ? '⏳ Generating...' : '🧪 Test Urdu TTS (Ayat 1)'}
              </button>
              {zTest.result && (
                <div style={{ marginTop:8, padding:'8px 14px', background:'rgba(46,139,87,0.08)', border:'1px solid rgba(46,139,87,0.2)', borderRadius:8 }}>
                  <div style={{ fontSize:11, color:'#2ecc71' }}>{zTest.result}</div>
                </div>
              )}
              {zTest.error && (
                <div style={{ marginTop:8, padding:'8px 14px', background:'rgba(231,76,60,0.08)', border:'1px solid rgba(231,76,60,0.2)', borderRadius:8 }}>
                  <div style={{ fontSize:11, color:'#e74c3c' }}>❌ {zTest.error}</div>
                  <div style={{ fontSize:10, color:'rgba(231,76,60,0.5)', marginTop:3 }}>Make sure REACT_APP_ZYPHRA_KEY is in .env file</div>
                </div>
              )}
            </div>
            ...
            {selected.number !== 9 && (
              <div style={{ textAlign: 'center', padding: '18px 26px 4px' }}>
                <div className="arabic q-bismillah" style={{ fontSize: 22, color: '#C9A84C', fontWeight: 600 }}>بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ</div>
              </div>
            )}

            {loading ? (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 60, gap: 12 }}>
                <Loader size={26} color="#C9A84C" style={{ animation: 'spin 1s linear infinite' }} />
                <p style={{ color: 'rgba(242,237,228,0.3)', fontSize: 13 }}>Loading {selected.englishName}...</p>
              </div>
            ) : (
              <div style={{ padding: '10px 26px 40px' }}>
                {ayats.map((ayat, i) => {
                  ...
                  return (
                    <div key={`${selected.number}-${ayat.numberInSurah}`} style={{ background: isPlaying ? 'rgba(201,168,76,0.06)' : '#0F0F0D', border: `1px solid ${isPlaying ? 'rgba(201,168,76,0.3)' : 'rgba(201,168,76,0.08)'}`, borderRadius: 14, padding: '16px 20px', marginBottom: 9, transition: 'background 0.3s, border-color 0.3s', animation: 'fadeInUp 0.35s ease forwards', animationDelay: `${Math.min(i*0.02,0.3)}s`, opacity: 0 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                        <div style={{ width: 26, height: 26, background: 'rgba(201,168,76,0.1)', border: '1px solid rgba(201,168,76,0.2)', borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: '#C9A84C' }}>{ayat.numberInSurah}</div>
                        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                          {isPlaying && <div style={{ display: 'flex', gap: 2 }}>{[0,1,2].map(j => <div key={j} style={{ width: 3, height: 12, background: '#C9A84C', borderRadius: 2, animation: `pulse 0.8s ease ${j*0.15}s infinite` }} />)}</div>}
                          {!S.current.isParah && (
                            <>
                              {ayat.numberInSurah > 1 && <button onClick={() => { stopAll(); setTimeout(() => { S.current = {...S.current, stopped: false, active: true, isParah: false, surahsData: [{surahNumber: selected.number, surahInfo: selected, ayahs: ayats, enTrans: translations, urTrans: urduTrans}], sIdx: 0, aNum: ayat.numberInSurah-1}; playAyatItem(0, ayat.numberInSurah-1); }, 80); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(242,237,228,0.3)' }}><SkipBack size={12}/></button>}
                              <button onClick={() => { if (isPlaying) stopAll(); else { stopAll(); setTimeout(() => { S.current = {...S.current, stopped: false, active: true, isParah: false, surahsData: [{surahNumber: selected.number, surahInfo: selected, ayahs: ayats, enTrans: translations, urTrans: urduTrans}], sIdx: 0, aNum: ayat.numberInSurah}; playAyatItem(0, ayat.numberInSurah); }, 80); }}} style={{ background: isPlaying ? 'rgba(46,139,87,0.25)' : 'rgba(46,139,87,0.1)', border: '1px solid rgba(201,168,76,0.15)', borderRadius: 6, padding: '4px 8px', cursor: 'pointer', color: '#C9A84C', display: 'flex', alignItems: 'center', gap: 3 }}>
                              ...
                              {ayat.numberInSurah < ayats.length && <button onClick={() => { stopAll(); setTimeout(() => { S.current = {...S.current, stopped: false, active: true, isParah: false, surahsData: [{surahNumber: selected.number, surahInfo: selected, ayahs: ayats, enTrans: translations, urTrans: urduTrans}], sIdx: 0, aNum: ayat.numberInSurah+1}; playAyatItem(0, ayat.numberInSurah+1); }, 80); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(242,237,228,0.3)' }}><SkipForward size={12}/></button>}
                            </>
                          )}
                        </div>
                      </div>
                      <div className="arabic" style={{ fontSize: 'clamp(20px,2.5vw,26px)', color: '#E8C97A', lineHeight: 2.2, marginBottom: 10, paddingBottom: 10, borderBottom: '1px solid rgba(201,168,76,0.08)' }}>
                        {/* ✅ Ayat 1 has Bismillah prepended by API - strip it */}
                        {(() => {
                          const t = ayat.text;
                          if (ayat.numberInSurah !== 1) return t;
                          // Bismillah is always 4 words: بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ
                          // Split by space and skip first 4 words
                      ...
                      {(displayLang === 'english' || displayLang === 'both') && translations[i] && (
                        <div style={{ fontSize: 13, color: isPlaying ? 'rgba(242,237,228,0.7)' : 'rgba(242,237,228,0.4)', lineHeight: 1.75, fontStyle: 'italic', marginBottom: displayLang === 'both' ? 6 : 0, transition: 'color 0.3s' }}>{translations[i].text}</div>
                      )}
                      {(displayLang === 'urdu' || displayLang === 'both') && urduTrans[i] && (
                        <div>
                          <div className="arabic" style={{ fontSize:14, color: isPlaying ? 'rgba(242,237,228,0.7)' : 'rgba(242,237,228,0.5)', lineHeight:2, transition:'color 0.3s', marginBottom:6 }}>
                            {urduTrans[i].text}
                          </div>
                          {/* 🔊 Urdu TTS Button */}
                          <button
                            onClick={async (e) => {
                              ...
                              if (urduTTSLoading === i) {
                                ...
                                setUrduTTSLoading(null);
                                return;
                              }
                              setUrduTTSLoading(i);
                              const blobUrl = await speakUrduTTS(urduTrans[i].text);
                              if (blobUrl) {
                                urduAudioRef.current.pause();
                                urduAudioRef.current.src = blobUrl;
                                urduAudioRef.current.play().catch(console.error);
                                urduAudioRef.current.onended = () => setUrduTTSLoading(null);
                              } else {
                                setUrduTTSLoading(null);
                              }
                            }}
                            style={{ display:'flex', alignItems:'center', gap:5, padding:'4px 10px', background: urduTTSLoading===i ? 'rgba(201,168,76,0.2)' : 'rgba(201,168,76,0.06)', border:`1px solid ${urduTTSLoading===i ? 'rgba(201,168,76,0.4)' : 'rgba(201,168,76,0.15)'}`, borderRadius:6, color:'#C9A84C', fontSize:10, cursor:'pointer', fontFamily:'inherit' }}
                          >
                            {urduTTSLoading === i ? <><Pause size={10}/> Stop</> : <><Volume2 size={10}/> اردو سنیں</>}
                          </button>
                        </div>
                      )}
                    </div>
      ...
      {/* ── Audio Bar ── */}
      {playing && selected && (
        <div style={{ position: 'fixed', bottom: 0, left: 72, right: 0, background: 'linear-gradient(135deg, #050505, #0F0F0D)', border: '1px solid rgba(212,175,55,0.2)', padding: '11px 22px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', zIndex: 200, boxShadow: '0 -8px 32px rgba(0,0,0,0.4)' }}>
